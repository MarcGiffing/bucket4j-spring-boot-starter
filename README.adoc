...
bucket4j:
  ...
  - cache-name: buckets   
    ...
    - filter-key-type: expression
      skip-condition:  @securityService.isSignedIn() # only for not logged in users
      ...
      - capacity: 10
        ...
        unit: minutes
    - filter-key-type: expression
      skip-condition: "!@securityService.isSignedIn() || @securityService.username() == 'admin'" # skip if not logged or not user admin 
      ...
      - capacity: 1000
        ...
        unit: minutes
    - filter-key-type: expression
      skip-condition:  "!@securityService.isSignedIn() && @securityService.username() != 'admin'"  # skip if not loggedin and user not admin => admin user
      expression: @securityService.username()
      bandwidths:
      - capacity: 1000000000
        time: 1
