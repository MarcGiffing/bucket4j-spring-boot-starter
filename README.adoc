...
The *first* is the default strategy. This the default strategy which only execute one rate limit.
So if you check in the first rate limit if the user is authenticated and the user is not you know that the other rate-limits handles
authenticated users.
...
Conditional filtering depending on the anonymous or logged in user. Cause the *bucket4j.filters[0].strategy* is *first*
you havn't to check in the second rate-limit if the user is logged in cause the first rate-limit check if the user is unauthenticated.
If its the case no other rate-limit will be evaluated.
