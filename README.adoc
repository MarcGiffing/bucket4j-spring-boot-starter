...
* > 0.20.0 - Bucket4j 8.16.x - Spring Boot 4.0.0 - Restructure in Maven Modules
...
Installing Bucket4j Spring Boot Starter requires **two dependencies**:
...
* *Synchronous* - Blocking operations using traditional servlet filters or method-level AOP interception
  - `bucket4j-spring-boot-starter-servlet` - Servlet Filter (blocking)
  - `bucket4j-spring-boot-starter-aop` - Method Annotations via AOP (blocking)

* *Asynchronous/Reactive* - Non-blocking operations for reactive applications
  - `bucket4j-spring-boot-starter-webflux` - Webflux WebFilter (non-blocking)
