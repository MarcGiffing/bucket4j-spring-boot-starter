package com.giffing.bucket4j.spring.boot.starter.core.utils;
...
public class Bucket4JUtils {
    ...
    public static ResponseEntity<String> validateConfigurationUpdate(Bucket4JConfiguration oldConfig, Bucket4JConfiguration newConfig) {
        ...
        if (oldConfig.getBucket4JVersionNumber() >= newConfig.getBucket4JVersionNumber()) {
            return ResponseEntity.badRequest().body("The new configuration (%s) should have a higher version  than the current configuration (%s)."
                    .formatted(newConfig.getBucket4JVersionNumber(), oldConfig.getBucket4JVersionNumber()));
        }
    ...
    private static ResponseEntity<String> validateFieldEquality(String fieldName, Object oldValue, Object newValue) {
        if (!Objects.equals(oldValue, newValue)) {
            String errorMessage = "It is not possible to modify the %s of an existing filter. Expected the field to be '%s' but is '%s'.".formatted(
