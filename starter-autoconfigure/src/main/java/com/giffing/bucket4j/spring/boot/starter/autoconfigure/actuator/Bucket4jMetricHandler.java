package com.giffing.bucket4j.spring.boot.starter.autoconfigure.actuator;
    ...
    public void handle(MetricType type, String name, long counterIncrement, List<MetricTagResult> tags) {
        ...
        switch (type) {
            case CONSUMED_COUNTER:
                Metrics.counter(METRIC_COUNTER_PREFIX + "consumed", extendedTagsArray)
                        .increment(counterIncrement);
                break;
            case REJECTED_COUNTER:
                Metrics.counter(METRIC_COUNTER_PREFIX + "rejected", extendedTagsArray)
                        .increment(counterIncrement);
                break;
            case PARKED_COUNTER:
                Metrics.counter(METRIC_COUNTER_PREFIX + "parked", extendedTagsArray)
                        .increment(counterIncrement);
                break;
            case INTERRUPTED_COUNTER:
                Metrics.counter(METRIC_COUNTER_PREFIX + "interrupted", extendedTagsArray)
                        .increment(counterIncrement);
                break;
            case DELAYED_COUNTER:
                Metrics.counter(METRIC_COUNTER_PREFIX + "delayed", extendedTagsArray)
                        .increment(counterIncrement);
                break;
            default:
